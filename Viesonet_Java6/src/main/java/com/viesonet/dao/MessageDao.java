...
public interface MessageDao extends JpaRepository<Message, Integer> {
	...
	@Query("SELECT DISTINCT m.sender.userId, m.sender.username, m.receiver.userId, m.receiver.username, m.sender.avatar, m.receiver.avatar, m.content,  m.sendDate FROM Message m WHERE m.receiver.userId = ?1 OR m.sender.userId = ?1 order by m.sendDate desc ")
